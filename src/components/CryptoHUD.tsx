      ...
      <div className="flex justify-between w-full pointer-events-auto">
        <div className="flex flex-col gap-3 w-72">
          <div className="bg-slate-900/90 backdrop-blur-md p-4 rounded-xl border border-slate-700 shadow-xl">
            ...
            <div className="mt-4 text-sm text-blue-400 font-bold flex justify-between">
        ...
        {/* Task Display */}
        <div className="bg-slate-900/90 backdrop-blur-md p-5 rounded-2xl border border-purple-500/50 shadow-[0_0_30px_rgba(168,85,247,0.3)] w-auto max-w-2xl">
          <div className="flex items-center gap-2 text-purple-300 mb-2 font-bold text-xl border-b border-slate-700 pb-2">
            <Shield /> {task.title}
          </div>
      ...
      <div className="pointer-events-auto flex justify-center mb-4 relative z-50">
        <div className="bg-slate-900/95 backdrop-blur-xl p-5 rounded-2xl border border-slate-700 shadow-2xl flex flex-col gap-3">
          ...
          {cryptoStatus === 'SUCCESS' ? (
            <div className="flex flex-col items-center justify-center p-6 gap-4">
              ...
              {!isGameOver && (
                <button 
                  ...
                  className="bg-purple-600 hover:bg-purple-500 text-white px-8 py-3 rounded-lg font-bold flex items-center gap-2 transition-transform hover:scale-105 active:scale-95"
                >
                  <Play /> 进入下一关
                </button>
              )}
            ...
            <>
              <div className="text-slate-400 text-sm font-bold text-center">选择并使用密钥 (Select Key to Apply)</div>
              <div className="grid grid-cols-5 gap-3">
                {keys.map((k) => (
                  <button
                    key={k.id}
                    onClick={() => submitCryptoKey(k.id)}
                    className="flex flex-col items-center justify-center p-4 rounded-lg border-2 border-slate-700 bg-slate-800 hover:bg-slate-700 hover:border-purple-500 transition-all text-slate-300 hover:text-white group active:scale-95"
                  >
                    {k.id === 'symmetric_key' ? <FileKey className="group-hover:text-amber-400 mb-2" size={32} /> : <Key className="group-hover:text-purple-400 mb-2" size={32} />}
                    <div className="text-sm font-bold text-center">{k.label}</div>
